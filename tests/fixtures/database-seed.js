...
async function seedSearchTestData(db) {
  ...
  await db.exec(`
    INSERT INTO parts (part_num, name, part_cat_id, ba_cat_id, image_available, sort_order) VALUES
    -- Parts with dimensions
    ('3001', 'Brick 2 x 4', 1, 10, 1, 1),
    ('3020', 'Plate 2 x 4', 2, 20, 1, 2),
    ('3021', 'Plate 2 x 3', 2, 20, 1, 3),
    ('3022', 'Plate 2 x 2', 2, 20, 1, 4),
    ('3034', 'Plate 2 x 8', 2, 20, 1, 5),
    ('3035', 'Plate 4 x 8', 2, 20, 0, 6),
    ('3032', 'Plate 4 x 6', 2, 20, 0, 7),
    
    -- Different dimension formats
    ('TEST01', 'Part 2x4 No Spaces', 1, 10, 0, 10),
    ('TEST02', 'Part 2 x 4 With Spaces', 1, 10, 0, 11),
    ('TEST03', 'Part 2×4 Unicode', 1, 10, 0, 12),
    ('TEST04', 'Part 2 × 4 Unicode Spaces', 1, 10, 0, 13),
    
    -- Multi-word search
    ('TEST10', 'Red Brick with Studs', 1, 10, 0, 20),
    ('TEST11', 'Blue Brick with Studs', 1, 10, 0, 21),
    ('TEST12', 'Red Plate with Clips', 2, 20, 0, 22),
    ('TEST13', 'Blue Plate with Rails', 2, 20, 0, 23),
    
    -- Special characters
    ('SPEC01', 'Part with "Quotes"', 1, 10, 0, 30),
    ('SPEC02', "Part with 'Apostrophe'", 1, 10, 0, 31),
    ('SPEC03', 'Part with & Ampersand', 1, 10, 0, 32);
  `)
}
...
async function seedCategoryTestData(db) {
  ...
  await db.exec(`
    ...
    ('P001', 'Part in A', 1, 1, 1, 1),
    ('P002', 'Part in B', 1, 2, 1, 2),
    ('P003', 'Part in C', 1, 3, 1, 3),
    
    -- Parts in subcategories
    ('P010', 'Part in A.1', 1, 10, 1, 10),
    ('P011', 'Part in A.2', 1, 11, 1, 11),
    ('P020', 'Part in B.1', 1, 20, 1, 20),
    
    -- Parts in deep subcategories
    ('P100', 'Part in A.1.1', 1, 100, 1, 100),
    ('P101', 'Part in A.1.2', 1, 101, 1, 101),
    ('P1000', 'Part in A.1.1.1', 1, 1000, 0, 1000),
    ('P1001', 'Part in A.1.1.2', 1, 1001, 0, 1001);
  `)
}
...
async function seedRelationshipTestData(db) {
  ...
  await db.exec(`
    ...
    ('CHAIN1', 'Chain Part 1', 1, 1, 0, 10),
    ('CHAIN2', 'Chain Part 2', 1, 1, 0, 11),
    ('CHAIN3', 'Chain Part 3', 1, 1, 0, 12),
    ('CHAIN4', 'Chain Part 4', 1, 1, 0, 13),
    
    -- Parts for circular relationships
    ('CIRC1', 'Circular Part 1', 1, 1, 0, 20),
    ('CIRC2', 'Circular Part 2', 1, 1, 0, 21),
    ('CIRC3', 'Circular Part 3', 1, 1, 0, 22);
  `)
...
async function seedPaginationTestData(db, partCount = 100) {
  ...
  for (let i = 1; i <= partCount; i++) {
    ...
    partInserts.push(`('${partNum}', '${name}', 1, 1, ${i % 3 === 0 ? 1 : 0}, ${i})`)
  }
