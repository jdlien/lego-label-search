...
async function seedMinimalTestData(db) {
  ...
  await db.exec(`
    INSERT INTO parts (part_num, name, part_cat_id, ba_cat_id, image_available, alt_part_ids, ba_name, sort_order) VALUES
    -- Basic bricks for testing
    ('3001', 'Brick 2 x 4', 1, 10, 1, NULL, 'Brick 2 x 4', 1),
    ('3002', 'Brick 2 x 3', 1, 10, 1, NULL, 'Brick 2 x 3', 2),
    ('3003', 'Brick 2 x 2', 1, 10, 1, NULL, 'Brick 2 x 2', 3),
    ('3004', 'Brick 1 x 2', 1, 10, 1, '3004b', 'Brick 1 x 2', 4),
    ('3004b', 'Brick 1 x 2 with Embossed Line', 1, 11, 0, '3004', 'Brick 1 x 2 with Line', 5),
    ('3005', 'Brick 1 x 1', 1, 10, 1, NULL, 'Brick 1 x 1', 6),
    
    -- Modified bricks for testing categories
    ('3622', 'Brick 1 x 3', 1, 10, 1, NULL, 'Brick 1 x 3', 7),
    ('87087', 'Brick 1 x 1 with Stud on Side', 1, 12, 1, NULL, 'Brick 1 x 1 with Stud on Side', 8),
    
    -- Plates for testing
    ('3020', 'Plate 2 x 4', 2, 20, 1, NULL, 'Plate 2 x 4', 10),
    ('3021', 'Plate 2 x 3', 2, 20, 1, NULL, 'Plate 2 x 3', 11),
    ('3022', 'Plate 2 x 2', 2, 20, 1, NULL, 'Plate 2 x 2', 12),
    ('3023', 'Plate 1 x 2', 2, 20, 1, NULL, 'Plate 1 x 2', 13),
    ('3024', 'Plate 1 x 1', 2, 20, 1, '3024b', 'Plate 1 x 1', 14),
    ('3024b', 'Plate 1 x 1 Round', 2, 21, 0, '3024', 'Plate 1 x 1 Round', 15),
    
    -- Plates with dimensions in name for search testing
    ('2420', 'Plate 2 x 2 Corner', 2, 21, 1, NULL, 'Plate 2 x 2 Corner', 16),
    ('3032', 'Plate 4 x 6', 2, 20, 0, NULL, 'Plate 4 x 6', 17),
    ('3035', 'Plate 4 x 8', 2, 20, 0, NULL, 'Plate 4 x 8', 18),
    
    -- Technic parts for testing
    ('3700', 'Technic Brick 1 x 2 with Hole', 4, 40, 1, NULL, 'Technic Brick 1 x 2 with Hole', 20),
    ('3701', 'Technic Brick 1 x 4 with Holes', 4, 40, 1, NULL, 'Technic Brick 1 x 4 with Holes', 21),
    ('2780', 'Technic Pin with Friction', 4, 41, 1, NULL, 'Technic Pin with Friction', 22),
    
    -- Parts with no images for testing
    ('99999', 'Test Part No Image', 1, 10, 0, NULL, 'Test Part No Image', 100),
    ('88888', 'Another Test No Image', 2, 20, 0, NULL, 'Another Test No Image', 101);
  `)
...
async function seedScenarioData(db, scenario) {
  switch (scenario) {
    case 'large-search-results':
      ...
      const partInserts = []
      for (let i = 1; i <= 100; i++) {
        partInserts.push(`('TEST${i.toString().padStart(4, '0')}', 'Test Part ${i}', 1, 10, 0, NULL, 'Test Part ${i}', ${100 + i})`)
      }
      await db.exec(`
        INSERT INTO parts (part_num, name, part_cat_id, ba_cat_id, image_available, alt_part_ids, ba_name, sort_order) VALUES
        ${partInserts.join(',\n')}
    ...
    case 'complex-relationships':
      ...
      await db.exec(`
        INSERT INTO parts (part_num, name, part_cat_id, ba_cat_id, image_available, alt_part_ids, ba_name, sort_order) VALUES
        ('REL001', 'Relationship Test 1', 1, 10, 0, NULL, 'Relationship Test 1', 200),
        ('REL002', 'Relationship Test 2', 1, 10, 0, NULL, 'Relationship Test 2', 201),
        ('REL003', 'Relationship Test 3', 1, 10, 0, NULL, 'Relationship Test 3', 202);
    ...
    case 'special-characters':
      ...
      await db.exec(`
        INSERT INTO parts (part_num, name, part_cat_id, ba_cat_id, image_available, alt_part_ids, ba_name, sort_order) VALUES
        ('SPEC001', 'Part with "Quotes"', 1, 10, 0, NULL, 'Part with "Quotes"', 300),
        ('SPEC002', "Part with 'Apostrophe'", 1, 10, 0, NULL, "Part with 'Apostrophe'", 301),
        ('SPEC003', 'Part with & Ampersand', 1, 10, 0, NULL, 'Part with & Ampersand', 302),
        ('SPEC004', 'Part with < > Brackets', 1, 10, 0, NULL, 'Part with < > Brackets', 303);
      `)
      break
  }
}
